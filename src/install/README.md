...
```sh
bash /path/to/br_pre_commit/src/install/install.sh "$PWD"
```

On Windows PowerShell:

```powershell
& "C:\path\to\br_pre_commit\src\install\install.ps1" `
...
```sh
bash br_pre_commit/src/install/install.sh "$PWD"
```

The installer resolves the absolute paths of the target repository and the
`br_pre_commit` checkout, then writes `.git/hooks/pre-commit`. The generated hook
records those paths in `BR_PRE_COMMIT_REPO_ROOT` and `BR_PRE_COMMIT_TOOL_ROOT`
and delegates to the repository-root `run` or `run.ps1` launcher.
