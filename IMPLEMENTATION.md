...
The installed hook delegates to `src/br_pre_commit/precommit_wrapper.py`. The wrapper reads the target project's `.pre-commit-config.yaml`, uses one shared staged-files context, runs file-mutating hooks serially, then runs read-only hooks in concurrent subprocesses. Unknown hooks fail by default (`wrapper.unknown-hook-policy` in `.br-pre-commit.toml`); `warn` runs them serially. Each job has the configured timeout.
